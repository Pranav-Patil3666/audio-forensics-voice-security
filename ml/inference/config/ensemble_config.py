...
class EnsembleConfig:
    # Report-derived baseline weights (CNN=0.5125, Wav2Vec2=0.4875)
    # score_i = 0.50 * ROC_AUC + 0.30 * REAL_F1 + 0.20 * REAL_RECALL
    # Rules = 0.0 until ml/inference/rules/ is wired into fusion
    cnn_weight:      float = env_float("SATARKRAHE_CNN_WEIGHT",      0.5125)
    wav2vec2_weight: float = env_float("SATARKRAHE_WAV2VEC2_WEIGHT", 0.4875)
    rules_weight:    float = env_float("SATARKRAHE_RULES_WEIGHT",    0.0)
